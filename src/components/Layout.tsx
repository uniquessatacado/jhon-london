...
const settingsNavItems = [
    ...
    { to: '/configuracoes/status-vendas', label: 'Status Vendas', icon: Activity, permissionKey: 'cadastros' as keyof UserPermissions },
...
export function Layout() {
  ...
  return (
    <div className="flex h-screen w-full bg-background overflow-hidden p-2 md:p-4 gap-4">
      ...
      <aside className="hidden md:flex flex-col w-72 rounded-3xl bg-card/80 backdrop-blur-xl border shadow-2xl overflow-hidden transition-all duration-300">
        <div className="flex h-24 items-center justify-center border-b px-4 bg-gradient-to-b from-accent/50 to-transparent">
           <AnimatedLogo />
        </div>
            ...
            {allowedNavItems.map(item => <NavLinkItem key={item.to} {...item} />)}
             ...
             {canAccessSettings && (
               <Collapsible defaultOpen={isSettingsOpen} className="mt-4">
                <CollapsibleTrigger className="flex items-center justify-between w-full rounded-2xl px-4 py-3 text-sm font-medium text-muted-foreground transition-all hover:bg-accent hover:text-foreground [&[data-state=open]>svg]:rotate-180 group">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full rounded-[2rem] bg-card/80 backdrop-blur-md border shadow-2xl overflow-hidden relative">
        
        <header className={`flex h-16 shrink-0 items-center gap-4 px-4 md:px-6 transition-all duration-300 z-10 sticky top-0 ${scrolled ? 'bg-background/80 backdrop-blur-xl border-b' : 'bg-transparent'}`}>
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="shrink-0 md:hidden text-foreground hover:bg-accent rounded-xl">
                <Menu className="h-6 w-6" />
                <span className="sr-only">Toggle navigation menu</span>
              </Button>
            </SheetTrigger>
            <SheetContent side="left" className="flex flex-col w-[85vw] max-w-sm bg-background/95 backdrop-blur-xl border-r text-foreground p-0">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 rounded-2xl bg-popover text-popover-foreground">
              <DropdownMenuLabel>Minha Conta</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuItem className="focus:bg-accent cursor-pointer" onClick={() => setIsProfileOpen(true)}>
                  Configurações
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem className="focus:bg-destructive/20 focus:text-destructive-foreground cursor-pointer" onClick={logout}>
