    ...
    try {
      ...
      for (const item of cart) {
        const { data: varData } = await supabase
          ...
          .eq('tamanho', item.variation.tamanho)
          .single();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md bg-zinc-950 border-white/10 p-0 overflow-hidden">
        <DialogHeader className="p-6 border-b border-white/10 bg-white/5">
          ...
          <DialogDescription>
            Confirme os detalhes e a forma de pagamento.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="p-6 space-y-6">
          <div className="bg-black/40 border border-white/10 rounded-xl p-4 space-y-3 shadow-inner">
             <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-medium text-white">{customer?.nome}</span>
             </div>
             <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground">Itens:</span>
                <span className="font-medium text-white">{cart.reduce((acc, item) => acc + item.quantity, 0)} unidades</span>
             </div>
             <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground">Subtotal:</span>
                <span className="font-mono text-white">{formatCurrency(totalItems)}</span>
             </div>
             ...
             <div className="flex justify-between items-center text-sm pt-2 border-t border-white/10">
                ...
                <Input 
                   type="number" 
                   min="0"
                   className="w-24 h-8 bg-black/60 border-white/10 text-right font-mono"
                   value={discount}
             ...
             <div className="flex justify-between items-center pt-2 border-t border-white/10">
                <span className="text-lg font-bold text-emerald-400">Total a Pagar:</span>
                <span className="text-2xl font-bold font-mono text-emerald-400">{formatCurrency(finalTotal)}</span>
             </div>
          </div>
          ...
          <div className="space-y-3">
             ...
             <div className="grid grid-cols-2 gap-3">
                <Button 
                   type="button" 
                   variant="outline" 
                   className={`h-14 flex flex-col items-center justify-center gap-1 transition-all ${paymentMethod === 'pix' ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400 ring-1 ring-emerald-500/50' : 'bg-black/40 border-white/10 text-muted-foreground hover:bg-white/5'}`}
                ...
                <Button 
                   type="button" 
                   variant="outline" 
                   className={`h-14 flex flex-col items-center justify-center gap-1 transition-all ${paymentMethod === 'dinheiro' ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400 ring-1 ring-emerald-500/50' : 'bg-black/40 border-white/10 text-muted-foreground hover:bg-white/5'}`}
                ...
                <Button 
                   type="button" 
                   variant="outline" 
                   className={`h-14 flex flex-col items-center justify-center gap-1 transition-all ${paymentMethod === 'credito' ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400 ring-1 ring-emerald-500/50' : 'bg-black/40 border-white/10 text-muted-foreground hover:bg-white/5'}`}
                ...
                <Button 
                   type="button" 
                   variant="outline" 
                   className={`h-14 flex flex-col items-center justify-center gap-1 transition-all ${paymentMethod === 'debito' ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400 ring-1 ring-emerald-500/50' : 'bg-black/40 border-white/10 text-muted-foreground hover:bg-white/5'}`}
        ...
        <DialogFooter className="p-4 border-t border-white/10 bg-black/20 gap-2">
          ...
          <Button 
             ...
             className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold w-full sm:w-auto px-8 shadow-lg shadow-emerald-500/20"
          >
